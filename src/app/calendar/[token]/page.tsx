...
export default function Calendar({ params }: { params: { token: string } }) {
  ...
  const { data: dataPublicProvider } = useQuery({
    queryKey: [REACT_QUERY_KEYS.provider.getPublicInfoProvider(params.token)],
    queryFn: () => apiProvider.getInfoProvider(params.token),
    ...{
